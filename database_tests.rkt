...
(test (SELECT * FROM Empty_table)
      '(()
        ()))

(test (SELECT * FROM Small)
  '(("Test")
    ("One")))
...
(test (SELECT * FROM [Empty_table "E"] [Empty_table "E"])
      '(()
        ()))
...
; Select some from two tables and the empty table
(test (SELECT '("P.Name" "Course" "Age") FROM [Person "P"] [Teaching "T"] [Empty_table "E"])
      '(("P.Name" "Course" "Age")
        ("David" "CSC324" 20)
        ("David" "CSC108" 20)
        ("David" "CSC343" 20)
...
; Take the product of a table with itself and the empty table
(test (SELECT '("E.Course" "E1.Course") FROM [Teaching "E1"] [Teaching "E"] [Empty_table "E"])
      '(("E.Course" "E1.Course")
        ("CSC324" "CSC324")
        ("CSC108" "CSC324")
        ("CSC343" "CSC324")
